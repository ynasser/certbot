...
ConfigureCronDeb() {
  if [ -z "$cron" ]; then
    ...
    ROOT_SPOOL_DIR=/var/spool/cron/crontabs/root/

    cerbot_cron_exists=$($SUDO grep "# generated by certbot" $ROOT_SPOOL_DIR)

    if [ -z "$cerbot_cron_exists" ]; then
      RANDOM_WAIT=$(($RANDOM % 3600))
      crontab_text="0 */12 * * * sleep RANDOM_WAIT ; certbot -q renew # generated by certbot\n"
      crontab_text=${$crontab_text/RANDOM_WAIT/$RANDOM_WAIT}
